...
export function useWebRTC(roomId: string) {
    ...
    const toggleScreenShare = async () => {
        ...
        // Block screen share softly via UI Banner if alone
        if (!pc) {
        ...
        if (isScreenSharing) {
            ...
            try {
                ...
                // Update local stream preview so user sees their own screen
                const stream = localStreamRef.current;
                if (stream) {
                    const oldTrack = stream.getVideoTracks()[0];
                    if (oldTrack) stream.removeTrack(oldTrack);
                ...
                screenTrack.onended = async () => {
                    screenTrackRef.current = null;
                    const camTrack = cameraTrackRef.current;
                    const endPc = pcRef.current; // Get fresh PC in case they joined mid-share
