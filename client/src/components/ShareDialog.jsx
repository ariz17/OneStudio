...
const socialPlatforms = [
    {
        ...
        getUrl: (link) => `https://wa.me/?text=${encodeURIComponent(`Join my OneStudios meeting: ${link}`)}`,
    },
    {
        name: "Telegram",
        ...
        getUrl: (link) => `https://t.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent("Join my OneStudios meeting!")}`,
    },
    {
        name: "Twitter",
        ...
        getUrl: (link) => `https://twitter.com/intent/tweet?text=${encodeURIComponent("Join my OneStudios meeting!")}&url=${encodeURIComponent(link)}`,
    },
    {
        name: "LinkedIn",
    ...
    },
    {
        ...
        getUrl: (link) => `mailto:?subject=${encodeURIComponent("Join my OneStudios Meeting")}&body=${encodeURIComponent(`Hey! Join my meeting on OneStudios:\n\n${link}`)}`,
    },
    {
        name: "SMS",
        icon: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z" /><path d="M8 12h.01" /><path d="M12 12h.01" /><path d="M16 12h.01" /></svg>),
        color: "bg-violet-500/10 text-violet-500 hover:bg-violet-500/20 border-violet-500/20",
        getUrl: (link) => `sms:?body=${encodeURIComponent(`Join my OneStudios meeting: ${link}`)}`,
    },
];

export function ShareDialog({ isOpen, onClose, roomId, inviteCode }) {
    const [copied, setCopied] = useState(null);

    if (!isOpen) return null;

    const baseUrl = typeof window !== "undefined" ? window.location.origin : "";
    const shareLink = inviteCode ? `${baseUrl}/join/${inviteCode}` : `${baseUrl}/call/${roomId}`;
                ...
                <div className="bg-background border border-border/60 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                    <div className="flex items-center justify-between p-5 border-b border-border/40">
                        <div className="flex items-center gap-3">
                            <div className="bg-primary/15 p-2.5 rounded-xl text-primary"><Share2 size={20} /></div>
                            <div>
                    ...
                    <div className="p-5 space-y-5">
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-2 p-1 bg-muted/50 border border-border/50 rounded-xl">
                                <input type="text" readOnly value={shareLink} className="flex-1 bg-transparent border-none outline-none px-3 py-2 text-sm font-mono truncate" />
                                <button onClick={() => handleCopy(shareLink, "link")} className={`shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${copied === "link" ? "bg-green-500/15 text-green-600" : "bg-primary text-primary-foreground hover:opacity-90"}`}>
                        ...
                        {inviteCode && (
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 p-1 bg-muted/50 border border-border/50 rounded-xl">
                                    <span className="flex-1 px-3 py-2 text-2xl font-bold tracking-[0.25em] text-center font-mono">{inviteCode}</span>
                                    <button onClick={() => handleCopy(inviteCode, "code")} className={`shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${copied === "code" ? "bg-green-500/15 text-green-600" : "bg-foreground text-background hover:opacity-90"}`}>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {socialPlatforms.map((platform) => (
                                    <button key={platform.name} onClick={() => handleShare(platform.getUrl)} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition-all duration-200 ${platform.color}`}>
                                        {platform.icon}
                        ...
                        <p className="text-[11px] text-muted-foreground text-center pt-1">Anyone with this link can join the meeting</p>
                    </div>
                </div>
            </div>
        </>
