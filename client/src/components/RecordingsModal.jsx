...
export function RecordingsModal({ isOpen, onClose, recordings }) {
    ...
    const handleDownloadAll = () => { for (const rec of recordings) { handleDownload(rec); } };
            ...
            <div className="bg-background border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="flex items-center justify-between p-5 border-b border-border">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">{recordings.length} recording{recordings.length !== 1 ? "s" : ""} received via P2P</p>
                        </div>
                    </div>
                    ...
                    {recordings.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {recordings.map((rec, i) => (
                                <div key={`${rec.name}-${i}`} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border hover:bg-muted/50 transition-colors group">
                                    ...
                                    <button onClick={() => handleDownload(rec)} className="p-2.5 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors shrink-0 opacity-70 group-hover:opacity-100" title="Download">
                                        <Download size={16} />
                                    </button>
                                </div>
                            ))}
                ...
                {recordings.length > 0 && (
                    <div className="p-5 border-t border-border flex items-center justify-between">
                        <p className="text-xs text-muted-foreground">Total: {formatBytes(recordings.reduce((sum, r) => sum + r.size, 0))}</p>
                        <button onClick={handleDownloadAll} className="flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-all">
