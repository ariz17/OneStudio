...
export function EmojiReactions({ onSendReaction, incomingReaction }) {
    const [floatingEmojis, setFloatingEmojis] = useState([]);
    const [showBar, setShowBar] = useState(false);

    const addFloating = useCallback((emoji, sender) => {
        ...
        <>
            ...
            <style>{`
                @keyframes emoji-float {
                    ...
                    50% { transform: translateY(-40vh) scale(1.3); opacity: 0.8; }
                    100% { transform: translateY(-80vh) scale(0.8); opacity: 0; }
                }
                .animate-emoji-float { animation: emoji-float 3s ease-out forwards; }
            `}</style>
        </>
    );
}
