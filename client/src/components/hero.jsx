...
export function Hero() {
  ...
  const handleJoinWithCode = async () => {
    ...
    try {
      ...
      if (room.type === "GROUP") { window.location.href = `/group/${roomId}` }
      else { window.location.href = `/call/${roomId}` }
    } catch {
      try {
        const room = await apiRequest(`/rooms/join/${roomId}`, {})
        if (room.type === "GROUP") { window.location.href = `/group/${room.id}` }
        else { window.location.href = `/call/${room.id}` }
      } catch {
      ...
      <div className="grid items-center gap-8 md:grid-cols-2">
        ...
        <div className="relative">
          <div className="rounded-[var(--radius-xl)] shadow-soft border border-border bg-card overflow-hidden">
            <img src="/images/hero-dashboard.jpg" alt="OneStudio dashboard preview" className="w-full h-auto" />
          </div>
        </div>
      </div>
    </section>
