...
export function VideoPlayer({ stream, label, isLocal = false, isMuted = false, isVideoOff = false, isScreenShare = false, backgroundClass }) {
    ...
    return (
        <div className={`video-container w-full h-full relative group shadow-sm rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-300 hover:ring-2 ring-ring ${backgroundClass || 'bg-muted/30 border border-border'}`}>
            <video
                ...
                playsInline
                muted={isLocal}
                className={`w-full h-full ${isScreenShare ? 'object-contain bg-black' : 'object-cover'} ${isVideoOff ? 'hidden' : ''}`}
            />
            {isVideoOff && (
                <div className="absolute inset-0 flex items-center justify-center bg-muted/60">
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-muted border border-border flex items-center justify-center">
                        <span className="text-xl md:text-2xl font-bold text-muted-foreground uppercase">{label.charAt(0)}</span>
                    </div>
                </div>
            )}
            {!isVideoOff && (
